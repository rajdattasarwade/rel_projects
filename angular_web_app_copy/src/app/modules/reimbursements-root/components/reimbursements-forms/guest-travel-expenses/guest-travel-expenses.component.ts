...
@Component({
  selector: 'app-guest-travel-expenses',
  templateUrl: './guest-travel-expenses.component.html',
  styleUrls: ['./guest-travel-expenses.component.css'],
  encapsulation: ViewEncapsulation.None
})
export class GuestTravelExpensesComponent implements OnInit {
  form: FormArray;
  parentForm: string;
  parentValue: any;
  obj = {
  ...
  ngOnInit(): void {
    ...
    if (this.data.payLoad.setOperation == 'Edit') {
      ...
      this.headerClaimDetails(); //call edit
    } else {
      this.statusText = false;
    ...
    this.reimbursmentService
        ...
        .subscribe(
          (data: ReimbursementsDetails[]) => {
            ...
            if (this.billDetailsData.length >0 && this.claimHeaderDetails.length >0) {
    ...
    this.reimbursmentService
      ...
      .subscribe(
        data => {
          ...
          this.form.controls[0]['controls'].traRequestNo.value =this.requestNumberNC;
        },
        err => {
          console.log(err);
        }
   ...
   */
  guestCreateAndEdit(totalAmt, opr) {
    let lineNumber = '';
    for (let [index, guestObj] of this.form.value.entries()) {
      totalAmt = totalAmt + parseInt(guestObj.requestedAmt);
      this.billDetails.attachDoc = null; //case of create
      this.billDetails.billNumber = guestObj.billNo;
      this.billDetails.billDate = new Date(guestObj.billDate).getTime();
      this.billDetails.billAmount = guestObj.requestedAmt;
      this.billDetails.remarks = guestObj.remarks;
      this.billDetails.requestedAmount = guestObj.requestedAmt;
      this.billDetails.claimNumber = this.claimNumber
        ? this.claimNumber
        : this.addAttachmentKey == 'A'
        ? ' '
        : '';
      this.billDetails.lineNumber = guestObj.lineNo;
  ...
  createGTPayLoad(index, opr) {
    ...
    if (opr == 'addAttachment') {
      ...
      this.guestCreateAndEdit(totalAmt, opr);
    }
  }
