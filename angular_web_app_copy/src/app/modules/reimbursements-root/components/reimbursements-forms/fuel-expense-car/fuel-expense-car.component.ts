...
@Component({
  selector: 'app-fuel-expense-car',
  templateUrl: './fuel-expense-car.component.html',
  styleUrls: ['./fuel-expense-car.component.css']
})
export class FuelExpenseCarComponent implements OnInit {
  fuelExpenseForm: FormGroup; //its a form group
  fuelExpenseFormArray: FormArray; // its a form array (supporting dynamic thinf)
  ...
  ngOnInit(): void {
    this.getClaimValue();
    this.fuelExpenseFormArray = new FormArray([]);
    ...
    if (this.data.payLoad.setOperation == 'Edit') {
          ...
          (data: ReimbursementsDetails[]) => {
            ...
            if (this.editClaimObj.length) {
              for (let editObj of this.editClaimObj) {
                ...
                this.fuelExpenseFormArray.push(
                  this.editfuelExpenseForm(editObj)
                );
              }
            }
          },
      ...
      this.addfuelExpenseForm();
    }
  }

  createfuelExpenseForm(counter) {
  ...
  addfuelExpenseForm() {
    if (this.editClaimObj.length > 0) {
      if (this.counter == 1) {
        ...
        this.fuelExpenseFormArray.push(
          this.createfuelExpenseForm(this.counter)
        );
      } else {
        this.fuelExpenseFormArray.push(
          this.createfuelExpenseForm(this.counter)
      ...
      this.fuelExpenseFormArray.push(this.createfuelExpenseForm(this.counter));
      this.counter++;
    }
  }
  ...
  editfuelExpenseForm(editObj) {
    ...
    this.fuelExpenseForm = new FormGroup({
      ...
      billAmount: new FormControl(
        { value: editObj.billAmount, disabled: viewMode },
        Validators.required
      ),
      expensePurpose: new FormControl({
  ...
  prepareFilesList(files: Array<any>, index) {
    if (files.length > 0) {
      if (files.length > 0) {
        ...
        this.fuelExpenseFormArray.controls[index]['controls'].attachment.value =
          files[0].name;
  ...
  addAttachmentFuctionality(index, totalAmt, opr) {
    ...
    let lineNumber = this.fuelExpenseFormArray.controls[index]['controls']
      .lineNo.value;
    this.claimObj.lineNumber = lineNumber;
    totalAmt =
      totalAmt +
      parseInt(
        this.fuelExpenseFormArray.controls[index]['controls'].billAmount.value
      );
    this.claimObj.attachDoc = null; //case of cremedicalObjate
    this.claimObj.billNumber = this.fuelExpenseFormArray.controls[index][
      'controls'
    ].billNo.value;
    this.claimObj.billDate = new Date(
      this.fuelExpenseFormArray.controls[index]['controls'].billDate.value
    ).getTime();
    this.claimObj.billAmount = this.fuelExpenseFormArray.controls[index][
      'controls'
    ].billAmount.value;
    this.claimObj.remarks = this.fuelExpenseFormArray.controls[index][
      'controls'
    ].expensePurpose.value;
    this.claimObj.requestedAmount = this.fuelExpenseFormArray.controls[index][
      'controls'
    ].billAmount.value;
  ...
  deleteAttachmentFunctionality(index, opr) {
    ...
    this.attachmentObj.lineNumber = this.fuelExpenseFormArray.controls[index][
      'controls'
    ].lineNo.value;
  ...
  createAndEdit(totalAmt, opr) {
    let lineNumber = '';
    for (let [index, medicalObj] of this.fuelExpenseFormArray.value.entries()) {
  ...
  deletefuelExpenseForm(index) {
    let lineNumber = '';
    // if(this.data.payLoad.setOperation=="Edit" && index < this.editClaimObj.length ){
    //   lineNumber = this.editClaimObj[index].lineNumber;
    //   } else {
    lineNumber = this.fuelExpenseFormArray.controls[index]['controls'].lineNo
      .value;
    // }
    this.fuelExpenseFormArray.removeAt(index);
  ...
  fuelExpenseFormSaveSubmitConfirmation(actionOpr) {
  ...
  checkIfDuplicateEntryPresent() {
    let lineNoForDuplicateVal = this.reimbursmentService.checkDuplicateTypeVal(
      this.fuelExpenseFormArray.value
    );
      ...
      data => {
        if (data.length > 0) {
          for (let item of data) {
            if (item.reimbursementType.sapCode == 'ZFMC') {
