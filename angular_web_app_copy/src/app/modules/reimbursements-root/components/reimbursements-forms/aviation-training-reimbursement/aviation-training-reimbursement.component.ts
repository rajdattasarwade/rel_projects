...
@Component({
  selector: 'app-aviation-training-reimbursement',
  templateUrl: './aviation-training-reimbursement.component.html',
  styleUrls: ['./aviation-training-reimbursement.component.css'],
  encapsulation: ViewEncapsulation.None,
})
export class AviationTrainingReimbursementComponent implements OnInit {
  subscriptionList: Subscription[] = [];
  aviationTrainingForm: FormGroup; //its a form group
  trainingFormArray: FormArray; // its a form array (supporting dynamic thinf)
  ...
  ngOnInit(): void {
    this.trainingFormArray = new FormArray([]);
    ...
    if (this.data.payLoad.setOperation == 'Edit') {
            ...
            if (this.editClaimObj.length) {
              for (let editObj of this.editClaimObj) {
                ...
                this.trainingFormArray.push(
                  this.editAviationTrainingForm(editObj)
                );
              }
            }
          },
      ...
      this.addAviationTrainingForm();
    }
  }

  createAviationTrainingForm(counter) {
    this.aviationTrainingForm = new FormGroup({
    ...
    return this.aviationTrainingForm;
  }

  addAviationTrainingForm() {
    if (this.editClaimObj.length > 0) {
      if (this.counter == 1) {
        ...
        this.trainingFormArray.push(
          this.createAviationTrainingForm(this.counter)
        );
      } else {
        this.trainingFormArray.push(
          this.createAviationTrainingForm(this.counter)
  ...
  editAviationTrainingForm(editObj) {
  ...
  prepareFilesList(files: Array<any>, index) {
    if (files.length > 0) {
      ...
      if (isFileSizeValid && isFileTypeValid) {
        ...
        this.trainingFormArray.controls[index]['controls'].attachment.value =
          files[0].name;
        this.trainingFormArray.controls[index].patchValue({
  ...
  addAttachmentFuctionality(index, totalAmt, opr) {
    ...
    let lineNumber = this.trainingFormArray.controls[index]['controls'].lineNo
      .value;
    this.claimObj.lineNumber = lineNumber;
    totalAmt =
      totalAmt +
      parseInt(
        this.trainingFormArray.controls[index]['controls'].billAmount.value
      );
    this.claimObj.attachDoc = null; //case of cretrainingObjate
    this.claimObj.billNumber = this.trainingFormArray.controls[index][
      'controls'
    ].billNo.value;
    this.claimObj.billDate = new Date(
      this.trainingFormArray.controls[index]['controls'].billDate.value
    ).getTime();
    this.claimObj.billAmount = this.trainingFormArray.controls[index][
      'controls'
    ].billAmount.value;
    this.claimObj.remarks = this.trainingFormArray.controls[index][
      'controls'
    ].expensePurpose.value;
    this.claimObj.requestedAmount = this.trainingFormArray.controls[index][
      'controls'
    ].billAmount.value;
    ...
    //   } else {
    //     this.claimObj.lineNumber = '000' + this.trainingFormArray.controls[index]['controls'].lineNo.value;
    //   }
  ...
  deleteAttachmentFunctionality(index, opr) {
    ...
    //   } else {
    // if(this.trainingFormArray.controls[index] != undefined){
    this.attachmentObj.lineNumber = this.trainingFormArray.controls[index][
      'controls'
    ].lineNo.value;
  ...
  createAndEdit(totalAmt, opr) {
    let lineNumber = '';
    for (let [index, trainingObj] of this.trainingFormArray.value.entries()) {
      totalAmt = totalAmt + parseInt(trainingObj.billAmount);
      this.claimObj.attachDoc = null; //case of create
      this.claimObj.billNumber = trainingObj.billNo;
      this.claimObj.billDate = new Date(trainingObj.billDate).getTime();
      this.claimObj.billAmount = trainingObj.billAmount;
      this.claimObj.remarks = trainingObj.expensePurpose;
      this.claimObj.requestedAmount = trainingObj.billAmount;
      this.claimObj.claimNumber = this.claimNumber
        ? this.claimNumber
        : this.addAttachmentKey == 'A'
        ? ' '
      ...
      //   this.claimObj.lineNumber = this.editClaimObj[index].lineNumber;
      //   } else {
      this.claimObj.lineNumber = trainingObj.lineNo;
      // }
  ...
  deleteAviationTrainingForm(index) {
    let lineNumber = '';
    // if(this.data.payLoad.setOperation=="Edit" && index < this.editClaimObj.length ){
    //   lineNumber = this.editClaimObj[index].lineNumber;
    //   } else {
    lineNumber = this.trainingFormArray.controls[index]['controls'].lineNo
      .value;
    // }
