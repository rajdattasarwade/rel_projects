...
@Component({
  selector: 'app-fuel-expense-two-wheeler',
  templateUrl: './fuel-expense-two-wheeler.component.html',
  styleUrls: ['./fuel-expense-two-wheeler.component.css'],
  encapsulation: ViewEncapsulation.None,
})
export class FuelExpenseTwoWheelerComponent implements OnInit {
  twowheelerForm: FormGroup; //its a form group
  twoWheelerFormArray: FormArray; // its a form array (supporting dynamic thinf)
  ...
  ngOnInit(): void {
    this.getClaimValue();
    this.twoWheelerFormArray = new FormArray([]);
    ...
    if (this.data.payLoad.setOperation == 'Edit') {
          ...
          (data: ReimbursementsDetails[]) => {
            ...
            if (this.editClaimObj.length) {
              for (let editObj of this.editClaimObj) {
                ...
                this.twoWheelerFormArray.push(this.editTwoWheelerForm(editObj));
              }
            }
          },
          (err) => {
      ...
      this.addTwoWheelerForm();
    }
  }

  createTwoWheelerForm(counter) {
    this.twowheelerForm = new FormGroup({
  ...
  addTwoWheelerForm() {
    if (this.editClaimObj.length > 0) {
      if (this.counter == 1) {
        ...
        this.twoWheelerFormArray.push(this.createTwoWheelerForm(this.counter));
      } else {
        this.twoWheelerFormArray.push(this.createTwoWheelerForm(this.counter));
        this.counter++;
      }
  ...
  prepareFilesList(files: Array<any>, index) {
    ...
    if (files.length > 0) {if (files.length > 0) {
      ...
      this.twoWheelerFormArray.controls[index]['controls'].attachment.value =
        files[0].name;
  ...
  addAttachmentFuctionality(index, totalAmt, opr) {
    ...
    let lineNumber = this.twoWheelerFormArray.controls[index]['controls'].lineNo
      .value;
    this.claimObj.lineNumber = lineNumber;
    totalAmt =
      totalAmt +
      parseInt(
        this.twoWheelerFormArray.controls[index]['controls'].billAmount.value
      );
    this.claimObj.attachDoc = null; //case of cremedicalObjate
    this.claimObj.billNumber = this.twoWheelerFormArray.controls[index][
      'controls'
    ].billNo.value;
    this.claimObj.billDate = new Date(
      this.twoWheelerFormArray.controls[index]['controls'].billDate.value
    ).getTime();
    this.claimObj.billAmount = this.twoWheelerFormArray.controls[index][
      'controls'
    ].billAmount.value;
    this.claimObj.remarks = this.twoWheelerFormArray.controls[index][
      'controls'
    ].expensePurpose.value;
    this.claimObj.requestedAmount = this.twoWheelerFormArray.controls[index][
      'controls'
    ].billAmount.value;
  ...
  deleteAttachmentFunctionality(index, opr) {
    ...
    this.attachmentObj.lineNumber = this.twoWheelerFormArray.controls[index][
      'controls'
    ].lineNo.value;
  ...
  createAndEdit(totalAmt, opr) {
    let lineNumber = '';
    for (let [index, medicalObj] of this.twoWheelerFormArray.value.entries()) {
  ...
  confirmAttachmentDelete(index) {
    ...
    this.twoWheelerFormArray.controls[index][
      'controls'
    ].attachment.value = '';
 this.attachedFiles[index] = null;
  }

  deleteTwoWheelerForm(index) {
    let lineNumber = '';
    // if(this.data.payLoad.setOperation=="Edit" && index < this.editClaimObj.length ){
    //   lineNumber = this.editClaimObj[index].lineNumber;
    //   } else {
    lineNumber = this.twoWheelerFormArray.controls[index]['controls'].lineNo
      .value;
    // }
    this.twoWheelerFormArray.removeAt(index);
  ...
  twoWheelerFormSaveSubmitConfirmation(actionOpr) {
  ...
  checkIfDuplicateEntryPresent() {
    let lineNoForDuplicateVal = this.reimbursmentService.checkDuplicateTypeVal(
      this.twoWheelerFormArray.value
    );
      ...
      (data) => {
        if (data.length > 0) {
          for (let item of data) {
            if (item.reimbursementType.sapCode == 'ZFMT') {
