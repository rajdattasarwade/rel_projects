...
export class ManagerReimbursementsComponent implements OnInit, OnDestroy {
      ...
      if (element.name === allSubObj.name) {
        if(allSubObj.dashboardModel.length == 0) {
          this.subscriptionList.push(
            this.expenseReportService.getdashboardClaims('R',this.currentYear, element.employeeNumber).subscribe((data: any) => { 
  ...
  getReimbursement(): void {
    ...
    this.subscriptionList.push(
      this.expenseReportService.getdashboardbyYear('R', this.currentYear).subscribe((data: DashboardModel[]) => {
        if(data) {
          this.userList = data;
 ...
 openReimburseMentModel(element, name): void {

  this.subscriptionList.push(
    this.expenseReportService.getClaimsList('R', this.currentYear, element.employeeNumber, element.expenseType).subscribe((data: ClaimsModel[]) => {
      let obj = {
        claimList: data,
        name: name,
        type: 'R',
      ...
      this.router.navigate(['/reimbursements/reimbursement-claims'], { state:obj });
    },error => {
    })
  );
